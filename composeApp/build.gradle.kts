    ...
    fun generate() {
        ...
        outDir.resolve("com/nuvio/app/features/simkl").apply {
            ...
            resolve("SimklConfig.kt").writeText(
                ...
                |    const val REDIRECT_URI = "${props.getProperty("SIMKL_REDIRECT_URI") ?: System.getenv("SIMKL_REDIRECT_URI") ?: "nuvio://auth/simkl"}"
                |    const val APP_NAME = "${props.getProperty("SIMKL_APP_NAME") ?: System.getenv("SIMKL_APP_NAME") ?: "nuvio"}"
                |}
                """.trimMargin()
            )
        }
...
val windowsPlayerBridgeCommand = if (missingWindowsPlayerBridgeInputs.isNotEmpty()) {
    ...
    val powershellCommand = """
        ...
        Set-Content ${'$'}bat -Value ${'$'}lines -Encoding ASCII
        & cmd.exe /d /c ${'$'}bat
        ${'$'}code = ${'$'}LASTEXITCODE
...
val generateRuntimeConfigs = tasks.register<GenerateRuntimeConfigsTask>("generateRuntimeConfigs") {
    ...
    privateSupabaseUrl.set(runtimeConfigValue("PRIVATE_SUPABASE_URL"))
    privateSupabaseAnonKey.set(runtimeConfigValue("PRIVATE_SUPABASE_ANON_KEY"))
    usePrivateSupabase.set(runtimeConfigBoolean("USE_PRIVATE_SUPABASE", false))
...
android {
    ...
    defaultConfig {
        applicationId = "com.nuvioenhanced.app"
    ...
    sourceSets.getByName("full") {
        ...
        java.srcDir("src/androidFull/kotlin")
    }
    packaging {
        resources {
