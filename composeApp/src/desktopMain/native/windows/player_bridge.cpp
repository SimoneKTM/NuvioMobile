...
public:
    ...
    void setVolumeBoost(double boostDb) {
        std::lock_guard<std::mutex> lock(mpvMutex);
        if (!mpv) return;
        double value = 100.0 + boostDb;
        mpvApi().setProperty(mpv, "volume", MPV_FORMAT_DOUBLE, &value);
    }
        ...
        while (!stopping.load()) {
            ...
            if (!event) {
                ...
                if (pollCount % 10 == 0) {
                    std::string err;
                    {
                        std::lock_guard<std::mutex> lock(mpvMutex);
                        if (mpv) {
                            char *fileError = nullptr;
                            if (mpvApi().getProperty(mpv, "file-error", MPV_FORMAT_STRING, &fileError) >= 0 && fileError) {
                                err = fileError;
                                mpvApi().freeValue(fileError);
                            }
                        }
                    }
                    if (!err.empty()) {
                        {
                            ...
                            if (lastError != err) {
                                lastError = err;
                                sendPlayerEvent("playbackError", 0.0);
                            }
                        }
                    }
                }
            ...
            if (event->event_id == MPV_EVENT_END_FILE) {
                ...
                if (endFile->reason == MPV_END_FILE_REASON_EOF) {
                    ...
                    {
                        std::lock_guard<std::mutex> lock(errorMutex);
                        std::string combined = "END_FILE_ERROR: " + errorMsg + " (reason=" + std::to_string(endFile->reason) + ")";
                        if (lastError != combined) {
                            lastError = combined;
                            sendPlayerEvent("playbackError", (double)endFile->reason);
                        }
                    }
                }
            }
...
extern "C" JNIEXPORT void JNICALL
Java_com_nuvio_app_features_player_desktop_NativePlayerBridge_setVolumeBoost(JNIEnv *, jobject, jlong handle, jfloat boostDb) {
    auto player = playerFromHandle(handle);
    if (player) player->setVolumeBoost(boostDb);
}
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_nuvio_app_features_player_desktop_NativePlayerBridge_errorMessage(JNIEnv *env, jobject, jlong handle) {
    auto player = playerFromHandle(handle);
    if (!player) return newJavaStringUtf8(env, "");
    std::lock_guard<std::mutex> lock(player->errorMutex);
    return newJavaStringUtf8(env, player->lastError);
}
