...
object SupabaseProvider {
    ...
    val client by lazy {
        val actualUrl = if (SupabaseConfig.USE_PRIVATE) SupabaseConfig.PRIVATE_URL else SupabaseConfig.URL
        val actualKey = if (SupabaseConfig.USE_PRIVATE) SupabaseConfig.PRIVATE_ANON_KEY else SupabaseConfig.ANON_KEY
        val userAgent = "NuvioMobile/${AppVersionConfig.VERSION_NAME.ifBlank { "dev" }}"
        createSupabaseClient(
            supabaseUrl = actualUrl,
            supabaseKey = actualKey,
        ) {
            httpConfig {
